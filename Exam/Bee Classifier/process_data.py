...
def generate_data():
    ...
    apis_data = apis_data.reshape(
        (num_apis_images, num_channels, size[0], size[1]))
    ...
    bombus_data = bombus_data.reshape(
        (num_bombus_images, num_channels, size[0], size[1]))
    ...
    all_apis_data = np.asarray(
        [apis_data] + [refl_apis_data] + orig_trans_apis_data + refl_trans_apis_data)
    all_bombus_data = np.asarray(
        [bombus_data] + [refl_bombus_data] + orig_trans_bombus_data + refl_trans_bombus_data)
